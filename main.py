from create_system import Elements, Planet, Company, DataAggregator, FuelPrices  # import my classes
...
number_of_planets = random.randint(20, 20)  # creates a number of planets, however many you want. try 1000 ;) (NO DON'T!)
...
planet_collection, company_collection = my_mongo_insert(
    number_of_planets,
    planets,
...
for tick in xrange(1, 500000):
    ...
    for planet in xrange(0, number_of_planets):  # cycles through each planet
        ...
        planets[planet].create_chart_values()
        if price_check >= 50:  # resets max price every 500 ticks, so max price is for example that days changes.
            for planet2 in xrange(0, number_of_planets):  # RESET FOR EVERY PLANET
                planets[planet2].reset_max_prices()
                price_check = 0
        if price_check >= 200:
            for company2 in xrange(0, number_of_planets):  # RESET FOR EVERY PLANET
                companies[company2].profit = 0
                price_check = 0
